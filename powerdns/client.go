...
func NewClient(serverURL string, apiKey string, nameserver string, configTLS *tls.Config, cacheEnable bool, cacheSizeMB string, cacheTTL int) (*Client, error) {
...
func (client *Client) ListZones() ([]ZoneInfo, error) {
	req, err := client.newRequest("GET", "/servers/"+client.Nameserver+"/zones", nil)
	if err != nil {
		return nil, err
	}
...
func (client *Client) GetZone(name string) (ZoneInfo, error) {
	req, err := client.newRequest("GET", fmt.Sprintf("/servers/"+client.Nameserver+"/zones/%s", name), nil)
	if err != nil {
		return ZoneInfo{}, err
	}
...
func (client *Client) CreateZone(zoneInfo ZoneInfo) (ZoneInfo, error) {
	...
	req, err := client.newRequest("POST", "/servers/"+client.Nameserver+"/zones", body)
	if err != nil {
		return ZoneInfo{}, err
	}
...
func (client *Client) UpdateZone(name string, zoneInfo ZoneInfoUpd) error {
	...
	req, err := client.newRequest("PUT", fmt.Sprintf("/servers/"+client.Nameserver+"/zones/%s", name), body)
	if err != nil {
		return err
	}
...
func (client *Client) DeleteZone(name string) error {
	req, err := client.newRequest("DELETE", fmt.Sprintf("/servers/"+client.Nameserver+"/zones/%s", name), nil)
	if err != nil {
		return err
	}
...
func (client *Client) ListRecords(zone string) ([]Record, error) {
	...
	if zoneInfo == nil {
		req, err := client.newRequest("GET", fmt.Sprintf("/servers/"+client.Nameserver+"/zones/%s", zone), nil)
		if err != nil {
			return nil, err
		}
...
func (client *Client) ReplaceRecordSet(zone string, rrSet ResourceRecordSet) (string, error) {
	...
	req, err := client.newRequest("PATCH", fmt.Sprintf("/servers/"+client.Nameserver+"/zones/%s", zone), reqBody)
	if err != nil {
		return "", err
	}
...
func (client *Client) setServerVersion() error {
	req, err := client.newRequest("GET", "/servers/"+client.Nameserver, nil)
	if err != nil {
		return err
	}
